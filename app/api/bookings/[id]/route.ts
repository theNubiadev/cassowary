...
export async function PATCH(
  ...
  switch (action) {
    case "ACCEPT":
      if (!isDriver)
        return NextResponse.json({ error: "Only the driver can accept" }, { status: 403 });
    ...
    case "DECLINE":
      if (!isDriver)
        return NextResponse.json({ error: "Only the driver can decline" }, { status: 403 });
      if (booking.status !== "PENDING")
    ...
    case "CANCEL":
      if (!isOwner)
        return NextResponse.json({ error: "Only the cargo owner can cancel" }, { status: 403 });
    ...
    case "MARK_IN_TRANSIT":
      if (!isDriver)
        return NextResponse.json({ error: "Only the driver can mark in transit" }, { status: 403 });
    ...
    case "MARK_DELIVERED":
      if (!isDriver)
        return NextResponse.json({ error: "Only the driver can mark delivered" }, { status: 403 });
      if (booking.status !== "IN_TRANSIT")
        return NextResponse.json({ error: "Booking must be in transit" }, { status: 400 });
